        ...
        while True:
            try:
                ...
                state = await loop.run_in_executor(
                    self._executor, self.manager.poll_status, None
                )
                lyric = ""
                visible = "false"
                if state and state.get("playback_status") == "playing":
                    lyrics_data = state.get("lyrics")
                    if lyrics_data:
                        current = lyrics_data.get("current_lyric")
                        if current:
                            lyric = current.split('|')[0].strip()
                            visible = "true" if lyric else "false"
                with open("/tmp/current_lyric.txt", "w") as f:
                    f.write(lyric)
                with open("/tmp/lyric_visible.txt", "w") as f:
                    f.write(visible)
            except Exception:
                pass
        ...
        try:
            async for message in websocket:
                try:
                    ...
                    loop = asyncio.get_running_loop()
                    state = await loop.run_in_executor(self._executor, self.manager.poll_status, requested_player)
        ...
        try:
            async for message in websocket:
                try:
                    ...
                    loop = asyncio.get_running_loop()
                    success = await loop.run_in_executor(self._executor, self._execute_control, action, player_name)
